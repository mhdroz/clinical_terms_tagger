...
def get_meddra_concepts_df(original_notes, nlp, matcher, window=10):
    """
...
def map_meddra_hier(df, meddraID_CUI_table, llt_to_pt, meddra_hier):

    print('mapping medID to ttype:')
    df['ttype'] = df['medID'].apply(lambda x: lookup_ttype(x, meddraID_CUI_table))
    ...
    df_hlt = df_hlt[['patid', 'note_id', 'date', 'note_title', 'concept_text', 'medID', #'extracted_CUI',
       'pos', 'context_left', 'context_right', 'ttype', 'PT', 'PT_text', #'PT_CUI',
       'HLT',
       'HLT_text', #'HLT_CUI', 
       'HLGT', 'HLGT_text', #'HLGT_CUI',
       'SOC', 'SOC_text', #'SOC_CUI'
       ]]
...
def normalize_dtype(df):
    ...
    #df = df.drop(['context_left', 'context_right'], axis=1)
